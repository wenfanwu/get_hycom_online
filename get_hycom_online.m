...
%     ncdisp(URL)  % debug
    ...
    if timeTick<min(timeAll)-dev_time || timeTick>max(timeAll)+dev_time
        error('The selected HYCOM product does not contain the data of the moment you specify, please check!')
    end

    if  min(lonAll)<0
        if max(region(1:2))>180
            ind_west = region>180;
            region(ind_west) = 180-region(ind_west);
        end
    end
...
elseif timeTick >= datetime(2012,1,25) && timeTick <= datetime(2013,8,20)    % checked    0-360, -80-80
